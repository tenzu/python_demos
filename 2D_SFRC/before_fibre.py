...
while F_CONT <= NUM_F:
    ...
    for i in range(0, SPL_SEG + 1):
        FC_CTR[i + (F_CONT - 1) * (SPL_SEG + 1), 0] = ORG_CTR[i, 0] * math.cos(theta_F * pi / 180.) - ORG_CTR[
            i, 1] * math.sin(theta_F * pi / 180.) + x_F
        FC_CTR[i + (F_CONT - 1) * (SPL_SEG + 1), 1] = ORG_CTR[i, 0] * math.sin(theta_F * pi / 180.) + ORG_CTR[
            i, 1] * math.cos(theta_F * pi / 180.) + y_F
    if F_CONT <= NUM_F:
        for j in range((F_CONT - 1) * (SPL_SEG + 1), F_CONT * (SPL_SEG + 1)):
            for k in range(1, (F_CONT - 1) * (SPL_SEG + 1)):
                if (FC_CTR[j, 0] - FC_CTR[k, 0]) ** 2 + (FC_CTR[j, 1] - FC_CTR[k, 1]) ** 2 > (
...
for i in range(len(F_LOCATIONS)):
    f2.write(
        '%11.5f' % F_LOCATIONS[i, 0] + ' ' + '%11.5f' % F_LOCATIONS[i, 1] + ' ' + '%11.5f' % F_LOCATIONS[i, 2] + '\n')
f2.close()
    ...
    if BA_CONT <= NUM_BA + 1:
        ...
        for i in range(0, BA_CONT - 1):
            if (BA_LOCATIONS[i, 0] - x_BA) ** 2 + (BA_LOCATIONS[i, 1] - y_BA) ** 2 > (
                    BA_LOCATIONS[i, 2] + BA_R) ** 2 + MIN_GAP:
                BA_AVLB = BA_AVLB * 1
            else:
                BA_AVLB = BA_AVLB * 0
    else:
