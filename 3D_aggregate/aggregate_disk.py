...
i, j = 0, 0
tmp_list, tmp_CSYS, move_CSYS, rotation_CSYS = np.zeros((6)), np.zeros((NUM_F, 6)), np.zeros((NUM_F, 3)), np.zeros(
    (NUM_F, 3))
...
while BA_CONT <= NUM_BA:
    ...
    z_BA = rdm.uniform(-(DISK_T / 2 - BA_R - MIN_GAP), DISK_T / 2 - BA_R - MIN_GAP)
    if BA_CONT <= NUM_BA + 1:
        for i in range(0, BA_CONT - 1):
            if (BA_LOCATIONS[i, 0] - x_BA) ** 2 + (BA_LOCATIONS[i, 1] - y_BA) ** 2 + (
                    BA_LOCATIONS[i, 2] - z_BA) ** 2 > (BA_LOCATIONS[i, 3] + BA_R) ** 2 + MIN_GAP:
                BA_AVLB = BA_AVLB * 1
            else:
                BA_AVLB = BA_AVLB * 0
    else:
...
for i in range(len(BA_LOCATIONS)):
    f1.write('%11.5f' % BA_LOCATIONS[i, 0] + ' ' + '%11.5f' % BA_LOCATIONS[i, 1] + ' ' + '%11.5f' % BA_LOCATIONS[
        i, 2] + ' ' + '%11.5f' % BA_LOCATIONS[i, 3] + '\n')
f1.close()
...
while BB_CONT <= NUM_BB:
    ...
    z_BB = rdm.uniform(-(DISK_T / 2 - BB_R - MIN_GAP), DISK_T / 2 - BB_R - MIN_GAP)
    if BB_CONT <= NUM_BB:
        for i in range(0, NUM_BA):
            if (BA_LOCATIONS[i, 0] - x_BB) ** 2 + (BA_LOCATIONS[i, 1] - y_BB) ** 2 + (
                    BA_LOCATIONS[i, 2] - z_BB) ** 2 > (BA_LOCATIONS[i, 3] + BB_R) ** 2 + MIN_GAP:
                BB_AVLB = BB_AVLB * 1
            else:
                BB_AVLB = BB_AVLB * 0
        for i in range(0, BB_CONT - 1):
            if (BB_LOCATIONS[i, 0] - x_BB) ** 2 + (BB_LOCATIONS[i, 1] - y_BB) ** 2 + (
                    BB_LOCATIONS[i, 2] - z_BB) ** 2 > (BB_LOCATIONS[i, 3] + BB_R) ** 2 + MIN_GAP:
                BB_AVLB = BB_AVLB * 1
            else:
                BB_AVLB = BB_AVLB * 0
    else:
...
for i in range(len(BB_LOCATIONS)):
    f1.write('%11.5f' % BB_LOCATIONS[i, 0] + ' ' + '%11.5f' % BB_LOCATIONS[i, 1] + ' ' + '%11.5f' % BB_LOCATIONS[
        i, 2] + ' ' + '%11.5f' % BB_LOCATIONS[i, 3] + '\n')
f1.close()
...
while BC_CONT <= NUM_BC:
    ...
    z_BC = rdm.uniform(-(DISK_T / 2 - BC_R - MIN_GAP), DISK_T / 2 - BC_R - MIN_GAP)
    if BC_CONT <= NUM_BC:
        for i in range(0, NUM_BA):
            if (BA_LOCATIONS[i, 0] - x_BC) ** 2 + (BA_LOCATIONS[i, 1] - y_BC) ** 2 + (
                    BA_LOCATIONS[i, 2] - z_BC) ** 2 > (BA_LOCATIONS[i, 3] + BC_R) ** 2 + MIN_GAP:
                BC_AVLB = BC_AVLB * 1
            else:
                BC_AVLB = BC_AVLB * 0
        for i in range(0, NUM_BB):
            if (BB_LOCATIONS[i, 0] - x_BC) ** 2 + (BB_LOCATIONS[i, 1] - y_BC) ** 2 + (
                    BB_LOCATIONS[i, 2] - z_BC) ** 2 > (BB_LOCATIONS[i, 3] + BC_R) ** 2 + MIN_GAP:
                BC_AVLB = BC_AVLB * 1
            else:
                BC_AVLB = BC_AVLB * 0
        for i in range(0, BC_CONT - 1):
            if (BC_LOCATIONS[i, 0] - x_BC) ** 2 + (BC_LOCATIONS[i, 1] - y_BC) ** 2 + (
                    BC_LOCATIONS[i, 2] - z_BC) ** 2 > (BC_LOCATIONS[i, 3] + BC_R) ** 2 + MIN_GAP:
                BC_AVLB = BC_AVLB * 1
            else:
                BC_AVLB = BC_AVLB * 0
    else:
...
for i in range(len(BC_LOCATIONS)):
    f1.write('%11.5f' % BC_LOCATIONS[i, 0] + ' ' + '%11.5f' % BC_LOCATIONS[i, 1] + ' ' + '%11.5f' % BC_LOCATIONS[
        i, 2] + ' ' + '%11.5f' % BC_LOCATIONS[i, 3] + '\n')
f1.close()
...
while BD_CONT <= NUM_BD:
    ...
    z_BD = rdm.uniform(-(DISK_T / 2 - BD_R - MIN_GAP), DISK_T / 2 - BD_R - MIN_GAP)
    if BD_CONT <= NUM_BD:
        for i in range(0, NUM_BA):
            if (BA_LOCATIONS[i, 0] - x_BD) ** 2 + (BA_LOCATIONS[i, 1] - y_BD) ** 2 + (
                    BA_LOCATIONS[i, 2] - z_BD) ** 2 > (BA_LOCATIONS[i, 3] + BD_R) ** 2 + MIN_GAP:
                BD_AVLB = BD_AVLB * 1
            else:
                BD_AVLB = BD_AVLB * 0
        for i in range(0, NUM_BB):
            if (BB_LOCATIONS[i, 0] - x_BD) ** 2 + (BB_LOCATIONS[i, 1] - y_BD) ** 2 + (
                    BB_LOCATIONS[i, 2] - z_BD) ** 2 > (BB_LOCATIONS[i, 3] + BD_R) ** 2 + MIN_GAP:
                BD_AVLB = BD_AVLB * 1
            else:
                BD_AVLB = BD_AVLB * 0
        for i in range(0, NUM_BC):
            if (BC_LOCATIONS[i, 0] - x_BD) ** 2 + (BC_LOCATIONS[i, 1] - y_BD) ** 2 + (
                    BC_LOCATIONS[i, 2] - z_BD) ** 2 > (BC_LOCATIONS[i, 3] + BD_R) ** 2 + MIN_GAP:
                BD_AVLB = BD_AVLB * 1
            else:
                BD_AVLB = BD_AVLB * 0
        for i in range(0, BD_CONT - 1):
            if (BD_LOCATIONS[i, 0] - x_BD) ** 2 + (BD_LOCATIONS[i, 1] - y_BD) ** 2 + (
                    BD_LOCATIONS[i, 2] - z_BD) ** 2 > (BD_LOCATIONS[i, 3] + BD_R) ** 2 + MIN_GAP:
                BD_AVLB = BD_AVLB * 1
            else:
                BD_AVLB = BD_AVLB * 0
    else:
...
for i in range(len(BD_LOCATIONS)):
    f1.write('%11.5f' % BD_LOCATIONS[i, 0] + ' ' + '%11.5f' % BD_LOCATIONS[i, 1] + ' ' + '%11.5f' % BD_LOCATIONS[
        i, 2] + ' ' + '%11.5f' % BD_LOCATIONS[i, 3] + '\n')
f1.close()
