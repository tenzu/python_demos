    ...
    for j in os.walk(i):
        for k in j[2]:  # Debug时发现j[2]存储了查重结果文件名，小幸运
            if 'pdf' in k:  # 剔除docx格式的查重文件
                PDF_Names.append(
                    Sub_Dir[Current_Dir.index(i)] + '_' +
                    k)  # 反查index，生成"教师姓名_学号_姓名_论文题目-文本复制检测报告单（全文标明引文）.pdf"list
for i in range(len(PDF_Names)):
    PDF_Names[i] = PDF_Names[i].strip("-文本复制检测报告单（全文标明引文）.pdf")  # 剔除无用str
    PDF_Names[i] = PDF_Names[i].split('_')  # 拆分list
for i in range(len(PDF_Names)):
    f.write(PDF_Names[i][2] + '\t' + PDF_Names[i][1] + '\t' + PDF_Names[i][3] +
            '\t' + PDF_Names[i][0] + '\n')
f.close()
